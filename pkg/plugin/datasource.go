...
func (d *Datasource) query(_ context.Context, _ backend.PluginContext, dataQuery backend.DataQuery) backend.DataResponse {
	...
	for {
		...
		if err != nil {
			...
			return backend.ErrDataResponse(backend.StatusBadRequest, fmt.Sprintf("query failed: %v", err.Error()))
		}
...
func (d *Datasource) CheckHealth(_ context.Context, req *backend.CheckHealthRequest) (*backend.CheckHealthResult, error) {
	...
	if len(d.Settings.CriblOrgBaseUrl) == 0 {
		res.Status = backend.HealthStatusError
		res.Message = "Cribl Organization URL must be defined"
		return res, nil
	}
